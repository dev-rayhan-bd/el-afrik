...
const placeClaimOrder = catchAsync(async (req: Request, res: Response) => {
  const { productId, pickupTime,uberQuoteId, uberFee } = req.body;
  const result = await BirthdayService.claimFreeOrder(req.user.userId, productId, pickupTime,uberQuoteId, uberFee);
